...
function NewsDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-['Manrope',system-ui,sans-serif]">
      <div className="progress-bar fixed top-0 left-0 h-1 bg-gray-900 w-0 z-50 transition-all duration-100"></div>

      <div className="max-w-[1200px] mx-auto px-5 py-4 text-[13.5px] text-gray-400">
        <Link to="/" className="text-gray-500 hover:text-gray-900 transition-colors">Bosh sahifa</Link>
        <span className="mx-2 text-gray-300">/</span>
        <Link to="/" className="text-gray-500 hover:text-gray-900 transition-colors">Yangiliklar</Link>
        <span className="mx-2 text-gray-300">/</span>
        <span>{news.title.slice(0, 40)}...</span>
      </div>

      <article className="max-w-[800px] mx-auto px-5 pb-10">
        <Link to="/" className="inline-flex items-center gap-1.5 text-gray-700 text-sm font-semibold mb-5 hover:gap-2.5 transition-all">
          ← Yangiliklar ro'yxatiga
        </Link>

        <span className="inline-block px-3.5 py-1.5 bg-gray-100 text-gray-700 text-[12.5px] font-bold rounded-full mb-4 uppercase tracking-wide">
          {news.category || "Tadbir"}
        </span>

        <h1 className="text-[26px] sm:text-[38px] font-extrabold text-gray-900 leading-tight tracking-[-0.02em] mb-4">
          {news.title}
        </h1>
        ...
        <div className="flex items-center gap-3.5 flex-wrap text-gray-500 text-[13.5px] pb-6 border-b border-gray-200 mb-7">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-gray-900 font-bold">Raqamli Avlod</span>
          </div>
        ...
        <div 
          className="text-[17px] leading-relaxed text-gray-700 space-y-5 [&_h2]:text-[25px] [&_h2]:font-extrabold [&_h2]:text-gray-900 [&_h2]:tracking-[-0.01em] [&_h2]:mt-8 [&_h2]:mb-3 [&_ul]:pl-5 [&_ul]:space-y-1 [&_ul]:list-disc [&_blockquote]:border-l-4 [&_blockquote]:border-gray-900 [&_blockquote]:py-3.5 [&_blockquote]:px-5 [&_blockquote]:my-8 [&_blockquote]:text-gray-700 [&_blockquote]:italic [&_blockquote]:font-medium [&_blockquote]:bg-gray-50 [&_blockquote]:rounded-r-xl [&_p]:mb-4"
        ...
        <div className="flex items-center gap-2.5 flex-wrap py-6 mt-8 border-t border-b border-gray-200">
          <span className="text-gray-500 text-sm font-bold mr-1">Ulashish:</span>
          <button onClick={copyLink} className="inline-flex items-center gap-1.5 px-3.5 py-2 border border-gray-300 rounded-lg text-gray-600 text-[13px] font-semibold bg-white hover:border-gray-900 hover:text-gray-900 hover:-translate-y-0.5 transition-all">
        ...
        <div className="max-w-[1200px] mx-auto px-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {relatedNews.map((item) => (
              <Link key={item.id} to={`/news/${item.slug}`} className="group bg-white border border-gray-200 rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-gray-300">
                ...
                <div className="p-5">
                  <h3 className="text-[15.5px] text-gray-900 mb-2 font-bold leading-snug line-clamp-2">{item.title}</h3>
